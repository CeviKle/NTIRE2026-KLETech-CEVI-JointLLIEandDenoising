...
if dataset in ['SID', 'SMID', 'SDSD_indoor', 'SDSD_outdoor']:
    ...
    with torch.inference_mode():
        for data_batch in tqdm(dataloader):
            ...
            restored = torch.clamp(restored, 0, 1).cpu().detach().permute(
                0, 2, 3, 1).squeeze(0).numpy()
            ...
            utils.save_img(os.path.join(result_dir, type_id, filename),
                           img_as_ubyte(restored))
            utils.save_img(os.path.join(result_dir_input, type_id, filename),
                           img_as_ubyte(input_save))
            utils.save_img(os.path.join(result_dir_gt, type_id, filename),
                           img_as_ubyte(target))
    ...
    with torch.inference_mode():
        for inp_path, tar_path in tqdm(zip(input_paths, target_paths), total=len(target_paths)):
            ...
            if output_dir != '':
                utils.save_img(os.path.join(output_dir, filename),
                               img_as_ubyte(restored))
            else:
                utils.save_img(os.path.join(result_dir, filename),
                               img_as_ubyte(restored))
...
# Retinexformer Testing Script (CPU + JPG input/output)
...
opt = parse(args.opt, is_train=False)
opt['dist'] = False   # IMPORTANT FIX

input_dir = opt['datasets']['val']['dataroot_lq']
gt_dir = opt['datasets']['val']['dataroot_gt']
...
print("Number of test images:", len(input_paths))
...
with torch.no_grad():
    ...
    for inp_path, gt_path in tqdm(zip(input_paths, gt_paths), total=len(input_paths)):
        ...
        psnr.append(utils.PSNR(gt, restored))
        ssim.append(utils.calculate_ssim(img_as_ubyte(gt), img_as_ubyte(restored)))

        filename = os.path.splitext(os.path.basename(inp_path))[0] + ".jpg"
